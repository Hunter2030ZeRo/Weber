...
[MENUBAR.md](../electron-runtime/MENUBAR.md). Outgoing header policy is implemented and verified in `8d426f7`;
see [REQUEST_HEADERS.md](../electron-runtime/REQUEST_HEADERS.md). HTTP protocol forwarding is implemented and verified in `f3c16b8`;
see [HTTP_PROTOCOL.md](../electron-runtime/HTTP_PROTOCOL.md). The `bd112a9` diagnostic separates termination output and identifies a prior
`isOffscreen` resource-policy failure; `3e1fb44` resolves that failure in both
VS Code layouts. The caught native-window-handle error remains. Readiness still
times out; its cause is not established by that caught message alone.
